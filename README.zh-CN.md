...
Canvas Mind Map 是一个 Obsidian 插件，提供两种独立 Rendering Mode：Native Canvas 保留原生卡片编辑体验；Organic Mind Map 使用独立 SVG 视图，以文字和曲线分支浏览 Markdown 文档结构。
...
| Organic Mind Map | 标题阅读、分支色彩、曲线连接、独立缩放和平移 | Markdown 文件右键或命令面板中的「以 Organic 模式打开笔记」 |

Organic 在独立标签页打开，不转换或覆盖已有 `.canvas`。单个顶层标题成为中心，多个顶层标题使用笔记名作为中心；无标题笔记显示笔记名。初始显示中心（depth=0）及下面两层标题。点击标题跳转到原笔记对应行，重复标题也能定位；点击标题下的 ＋/− 展开或收起子树。折叠时保留分支颜色、左右归属与顺序，并保持操作标题的屏幕位置。

拖动空白处平移，滚轮围绕鼠标缩放，工具栏可缩放或「适应窗口」。聚焦背景后也可使用方向键、`+`/`-` 和 `0`。标题和折叠按钮支持键盘聚焦；Enter 打开标题，Enter/空格切换折叠。

Organic 首期只显示标题，不提供自由编辑、手动创建节点或 Canvas 撤销。修改 Markdown 后点击「刷新」；原文发生变化时，跳转会提示先刷新，防止跳错行。视角和折叠仅保留在当前视图会话；关闭后重新打开恢复默认两层，不生成额外导图文件。两种模式不要求功能一致。
