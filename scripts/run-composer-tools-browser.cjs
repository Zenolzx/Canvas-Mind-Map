...
try{
 ...
 async function menu(title,group){await page.getByRole('button',{name:'More',exact:true}).click();if(group)await page.getByRole('menuitem',{name:group,exact:true}).click();await page.getByRole('menuitem',{name:title,exact:true}).click();}
 ...
 await nodeMenu('Planning','Collapse branch','Branch');assert.equal(await node('Goals').count(),0);await nodeMenu('Planning','Expand one level','Branch');assert.equal(await node('Goals').count(),1);
 await node('Risks').click();await page.getByRole('combobox',{name:'Node type',exact:true}).selectOption('todo');await page.getByRole('checkbox',{name:'Complete Risks',exact:true}).click();assert.equal((await state()).root.children.find(n=>n.title==='Risks').checked,true);
 await node('Milestones').click();await page.getByRole('combobox',{name:'Node type',exact:true}).selectOption('idea');await page.getByRole('button',{name:'Body',exact:true}).click();await page.locator('.cmm-composer-body textarea').fill('Explore dependencies');
 ...
 await page.getByRole('button',{name:/^Ideas(?: \d+)?$/,exact:false}).click();await page.getByRole('button',{name:'Add idea',exact:true}).click();await page.getByRole('textbox',{name:'New unsorted idea',exact:true}).fill('Later research');await page.getByRole('button',{name:'Confirm',exact:true}).click();assert.equal((await state()).unsorted[0].title,'Later research');
 await page.locator('.cmm-composer-unsorted-list button').dblclick();
 assert.equal(await page.locator('.cmm-composer-idea-input').evaluate(el=>document.activeElement===el && el.selectionEnd===el.value.length),true);
 await page.locator('.cmm-composer-idea-input').fill('Temporary');await page.keyboard.press('Escape');assert.equal((await state()).unsorted[0].title,'Later research');
 await page.keyboard.press('F2');await page.locator('.cmm-composer-idea-input').fill('Later research');await page.keyboard.press('Enter');assert.equal((await state()).unsorted.length,1);
 ...
 await page.getByRole('button',{name:'View',exact:true}).click();await page.getByRole('menuitem',{name:'Fit to view',exact:true}).click();await drag(page.locator('.cmm-composer-unsorted-list').getByRole('button',{name:'? Later research',exact:true}),node('Project Plan'));assert.equal((await state()).unsorted.length,0);
